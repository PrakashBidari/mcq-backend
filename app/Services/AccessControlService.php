...
class AccessControlService
{
    ...
    /**
     * Charge one unit for a quiz that is being started - an attempts purchase, an
     * attempts trial, or a wallet attempt. Called from GET /question-set/{id} the
     * moment the questions are handed to the client, so the cap is enforced even if
     * the client never reports the quiz as finished (app killed, lost network on
     * the save-attempt call, etc.). No-op for free content, day-based grants and
     * active subscriptions - those aren't "per play".
     *
     * $attemptKey is a per-start id generated by the client. The same key is only
     * ever charged once (6h window), so a re-fetch / retry / double request for the
     * one real start does not burn a second attempt. A fresh start sends a fresh
     * key and is charged normally.
     */
    public function consumeForQuizStart(
        ?User $user,
        ...
        // Resolve package-gated sets to their package so the dedupe key and the
        // consumption both act on the same entity.
        if ($item instanceof QuestionSet && $item->package_id) {
            $this->consumeForQuizStart(
                $user,
        ...
        if ($attemptKey) {
            ...
            $cacheKey = "quiz_start:{$user->id}:{$entity}:{$item->id}:{$attemptKey}";
            if (Cache::has($cacheKey)) {
                return; // this exact start was already charged
            }
            Cache::put($cacheKey, true, now()->addHours(6));
        }

        // Same priority logic as starting a quiz, WITH consumption (trial use /
