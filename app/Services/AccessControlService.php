...
class AccessControlService
{
    ...
    /**
     * Charge one unit for a completed quiz - an attempts purchase, an attempts trial,
     * or a wallet attempt. Called from POST /quiz/save-attempt (NOT when the questions
     * are fetched), so a re-render / retry / abandoned quiz / stacked post-purchase
     * "Start Quiz" prompt never silently spends a paid attempt. No-op for free content,
     * day-based grants and active subscriptions - those aren't "per play".
     *
     * $attemptKey is a per-start id generated by the client. The same key is only
     * ever charged once (24h window), so a resent save call for the one real play does
     * not burn a second attempt. A fresh play sends a fresh key and is charged normally.
     */
    public function consumeForQuizPlay(
        ?User $user,
        QuestionSet|QuestionSetPackage $item,
        ...
        // Resolve package-gated sets to their package FIRST - before the is_paid
        // check below. A package-exclusive question set carries is_paid = false on
        // its own row (it is gated entirely by the package), so checking is_paid
        // here first would bail out and never charge the package attempt, letting
        // the set be replayed forever. The package itself is the paid entity.
        if ($item instanceof QuestionSet && $item->package_id) {
            $this->consumeForQuizPlay(
                $user,
                $item->package ?? $item->package()->firstOrFail(),
                $attemptKey,
            );
            return;
        }
        ...
        if ($attemptKey) {
            $entity = $item instanceof QuestionSetPackage ? 'package' : 'set';
            $cacheKey = "quiz_play:{$user->id}:{$entity}:{$item->id}:{$attemptKey}";
            if (Cache::has($cacheKey)) {
                return; // this exact play was already charged
            }
            Cache::put($cacheKey, true, now()->addHours(24));
        }

        // Same priority logic as the access gate, WITH consumption (trial use /
        // wallet attempt / purchase attempts_used++). Days grants and subscriptions
        // fall through without being decremented.
        $this->evaluate($user, $item, consume: true);
    }
     ...
     *
     * Shape: ['kind' => 'attempts'|'trial_attempts'|'days'|'hours'|'minutes'|'trial_days'|'subscription'|'unlimited'|'wallet', ...]
     *  - attempts / trial_attempts: attempts_used, attempts_total, attempts_remaining
     *  - days / hours / minutes:    expires_at (ISO 8601), seconds_remaining,
     *                               minutes_remaining, hours_remaining, days_remaining
     *  - trial_days:                expires_at (ISO 8601), days_remaining
    ...
    public function accessSummary(?User $user, QuestionSet|QuestionSetPackage $item): ?array
    {
        ...
        if ($activePurchase) {
            if ($activePurchase->expires_at) {
                // 'days' | 'hours' | 'minutes' - the unit the admin sold this window in.
                // Fall back to 'days' for legacy rows that have an expiry but no type.
                $timeKind = in_array($activePurchase->access_type, ['days', 'hours', 'minutes'], true)
                    ? $activePurchase->access_type
                    : 'days';

                return $this->timeAccessSummary($timeKind, $activePurchase->expires_at);
            }
    ...
    /**
     * Build the "time left" summary for a day/hour/minute-based grant. `days_remaining`
     * is kept populated for every time kind so older clients that only understand
     * day-based access still show something sensible.
     *
     * @param  'days'|'hours'|'minutes'  $kind
     * @param  \Carbon\CarbonInterface  $expiresAt
     */
    private function timeAccessSummary(string $kind, $expiresAt): array
    {
        ...
        return [
            ...
            'minutes_remaining' => (int) ceil($secondsLeft / 60),
            'hours_remaining' => (int) ceil($secondsLeft / 3600),
            'days_remaining' => (int) ceil($secondsLeft / 86400),
        ];
    }
