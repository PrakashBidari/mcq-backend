    ...
    <div class="max-w-4xl">
        ...
        <div class="rounded-lg bg-white shadow-sm">
            <div class="border-b border-gray-200 p-6">
                <h3 class="text-xl font-bold text-gray-800">FAQ Information</h3>
                <p class="mt-1 text-sm text-gray-600">Fill in the details to create a new FAQ</p>
            </div>

            <form action="{{ route('faqs.store') }}" method="POST" class="space-y-6 p-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="answer" id="answer" rows="6" required
                        class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500"
                        placeholder="To reset your password, go to Settings...">{{ old('answer') }}</textarea>
                    @error('answer')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
