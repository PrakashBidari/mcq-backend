    ...
    <div class="max-w-3xl">
        ...
        <div class="rounded-lg bg-white shadow-sm">
            <div class="border-b border-gray-200 p-6">
                ...
                <p class="mt-1 text-sm text-gray-600">Fill in the details to create a new book</p>
            </div>
            ...
            <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500"
                        placeholder="This comprehensive guide covers the fundamental principles of design thinking and user-centered design...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="url" name="cover" id="cover" value="{{ old('cover') }}" required
                        class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500"
                        placeholder="https://images.unsplash.com/photo-xxxxx">
                    @error('cover')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Enter a valid image URL (Unsplash, etc.)</p>
                </div>

                <!-- Upload Image (Optional) -->
                <div>
                    <label for="image" class="mb-2 block text-sm font-semibold text-gray-700">
                        Or Upload Book Image (Optional)
                    </label>
                    ...
                    @error('image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Upload an image file (JPG, PNG, GIF, WebP - max 2MB). This will
                        override the URL if provided.</p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="difficulty" id="difficulty" required
                            ...
                            <option value="Beginner" {{ old('difficulty') == 'Beginner' ? 'selected' : '' }}>Beginner
                            </option>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="time" name="duration" id="duration" value="{{ old('duration') }}" required
                            class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500"
                            placeholder="12h 30m">
                        @error('duration')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
