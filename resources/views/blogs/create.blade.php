    ...
    <div class="max-w-4xl">
        ...
        <div class="rounded-lg bg-white shadow-sm">
            <div class="border-b border-gray-200 p-6">
                <h3 class="text-xl font-bold text-gray-800">Blog Information</h3>
                <p class="mt-1 text-sm text-gray-600">Fill in the details to create a new blog post</p>
            </div>

            <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6"
                id="blogForm" onsubmit="return validateContent()">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="excerpt" id="excerpt" rows="3" required
                        class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500"
                        placeholder="Brief summary of the blog post (shown in list view)">{{ old('excerpt') }}</textarea>
                    @error('excerpt')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10"
                        class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    @error('image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Upload an image file (JPG, PNG, GIF, WebP - max 2MB)</p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="read_time" id="read_time" value="{{ old('read_time', '5 min') }}"
                        required
                        class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500"
                        placeholder="5 min">
                    @error('read_time')
