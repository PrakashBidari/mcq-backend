...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white rounded-lg shadow-sm">
            <div class="p-6 border-b border-gray-200">
                <h3 class="text-xl font-bold text-gray-800">Edit Book Category</h3>
            </div>

            <form action="{{ route('book-categories.update', $category->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="flex items-center gap-3 pt-6 border-t border-gray-200">
                    <button type="submit" class="px-6 py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition">Update Book Category</button>
    ...
    <script>
        function bookCategoryIconPicker() {
            return {
                selectedIcon: '{{ old('icon', $category->icon ?? 'library') }}',
                icons: [
