...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white rounded-lg shadow-sm" x-data="{ name: '{{ old('name') }}', slug: '{{ old('slug') }}' }">
            <div class="p-6 border-b border-gray-200">
                <h3 class="text-xl font-bold text-gray-800">Blog Category Information</h3>
            </div>

            <form action="{{ route('blog-categories.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" x-model="name"
                        ...
                        required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500" placeholder="e.g., Study Tips">
                ...
                <div class="flex items-center gap-3 pt-6 border-t border-gray-200">
                    <button type="submit" class="px-6 py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition">Create Blog Category</button>
    ...
    <script>
        function blogCategoryIconPicker() {
            return {
                selectedIcon: '{{ old('icon', 'newspaper') }}',
                icons: [
