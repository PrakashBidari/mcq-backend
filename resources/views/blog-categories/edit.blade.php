...
@section('content')
    <div class="max-w-2xl">
        <div class="mb-6">
            <a href="{{ route('blog-categories.index') }}" class="inline-flex items-center text-purple-600 hover:text-purple-700">
                &larr; Back to Blog Categories
            </a>
        </div>

        <div class="bg-white rounded-lg shadow-sm">
            <div class="p-6 border-b border-gray-200">
                <h3 class="text-xl font-bold text-gray-800">Edit Blog Category</h3>
            </div>

            <form action="{{ route('blog-categories.update', $category->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">{{ old('description', $category->description) }}</textarea>
                ...
                <div>
                    ...
                    <input type="color" name="color" id="color" value="{{ old('color', $category->color) }}" class="h-12 w-20 rounded-lg border border-gray-300 cursor-pointer">
                    @error('color')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div x-data="blogCategoryIconPicker()">
                ...
                <div class="flex items-center gap-3 pt-6 border-t border-gray-200">
                    <button type="submit" class="px-6 py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition">Update Blog Category</button>
                    <a href="{{ route('blog-categories.index') }}" class="px-6 py-3 bg-gray-100 text-gray-700 font-semibold rounded-lg hover:bg-gray-200 transition">Cancel</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function blogCategoryIconPicker() {
            return {
                selectedIcon: '{{ old('icon', $category->icon ?? 'newspaper') }}',
                icons: [
                    'newspaper', 'document-text', 'book', 'library', 'bookmark',
